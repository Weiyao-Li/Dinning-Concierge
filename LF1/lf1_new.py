...
def validationProcess(Location, Cuisine, Date, Time, Numberofpeople, Email):
    ...
    if Date:
        ...
        if date_to_check < datetime.date.today():
            return build_validation_result(False, 'dining_date', 'SpellByWord', 'Please enter a valid Dining date')

    if Time:
        print("Debug: time is:", Time)
        if not Time:
            return build_validation_result(False, 'dining_time', 'SpellByWord', '')
    print('returningTrue!!!!!')
...
def DiningSuggestionsIntent(intent_request):
    ...
    Date = get_slot(intent_request, "dining_date")
    Time = get_slot(intent_request, "dining_time")
    Numberofpeople = get_slot(intent_request, "no_of_people")
    ...
    if source == 'DialogCodeHook':
        ...
        if not resOfValidation['isValid']:
            ...
            print("now eliciting")
            print(resOfValidation)
            result = elicit_slot(intent_request, session_attributes, resOfValidation['violatedSlot'],slots,resOfValidation['message'])
...
def send_message_to_SQS(Location, Cuisine, Date, Time, Numberofpeople, Email):
    ...
    response = sqs.send_message(
        QueueUrl="https://sqs.us-east-1.amazonaws.com/112295921782/RestaurantQueue",
        MessageAttributes={
            'Location': {
                'DataType': 'String',
